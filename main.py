        ...
        if len(top) < 2:
            ...
            for i in range(len(top) - 1):
                if top[i][0] > top[i + 1][0]:
                    print(f'Функция убывает на промежутке от {round(top[i][1], 2)} до {round(top[i+1][1], 2)}')
                else:
                    print(f'Функция возрастает на промежутке {round(top[i][1], 2)} до {round(top[i+1][1], 2)}')
